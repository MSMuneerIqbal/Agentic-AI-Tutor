...
- Q: How should quiz length be determined per session? → A: Bounded adaptive: target 15–20; stop early on mastery, extend to 20 if borderline.
...
3. Given consistent correct answers with high confidence, When the learner reaches at least 15 questions, Then the quiz may end early and be marked as pass.
4. Given borderline performance near the pass threshold, When the learner reaches 15 questions, Then the quiz extends up to 20 questions before deciding pass/fail.
...
- **FR-004**: The system MUST generate a bounded-adaptive quiz targeting 15–20 questions, delivering one question per turn with up to two hints per question; stop early on demonstrated mastery (≥15), and extend to 20 when performance is borderline.
